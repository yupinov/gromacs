                                                       ...
                                                       float         &fz)
{
    ...
    if (!(order & (order - 1))) // only for orders of power of 2
    {
        ...
        assert(atomDataSize <= warp_size);
        const int width = atomDataSize;
        // have to rework for atomDataSize > warp_size (order 8 or larger...)
...
#pragma unroll
    ...
    if ((outputIndexLocal < blockForcesSize) && globalOutputCheck)
    {
        ...
        assert(!isnan(gm_forces[outputIndexGlobal]));
    }
}
                    ...
                    )
{
    ...
    if (!pme_gpu_performs_FFT(pmeGpu) || pme_gpu_is_testing(pmeGpu))
    {
        ...
        cu_copy_H2D_async(kernelParamsPtr->grid.d_realGrid, const_cast<float *>(h_grid), pmeGpu->archSpecific->gridSize * sizeof(float), stream);
    }
