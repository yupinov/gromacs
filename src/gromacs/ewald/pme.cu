...
void pme_gpu_realloc_and_copy_bspline_values(const pme_gpu_t *pmeGPU)
{
    ...
    const size_t modSize = newSplineValuesSize * sizeof(float);
    for (int i = 0; i < DIM; i++)
    {
        ...
        /*FIME: no need for separate buffers */
...
void pme_gpu_sync_energy_virial(const pme_gpu_t *pmeGPU)
{
    ...
    {
        GMX_ASSERT(!isnan(pmeGPU->h_virialAndEnergy[j]), "PME GPU produces incorrect energy/virial.");
    }
}
