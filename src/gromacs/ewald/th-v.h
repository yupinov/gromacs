...
struct local_vectors 
{
    ...
    template <typename V> V &create_or_resize(void *&p, int size) 
    {
        if (p == NULL || (int) ((V *) p)->size() < size) 
        {
            ...
            else 
            {
                if (size > 0) 
                {
                    // unused warning, fixed in thrust 1.7.1 //yupinov
    ...
    template <typename T> thrust::host_vector<T> &host(int id, int size) 
    {
