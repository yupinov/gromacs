             ...
             int imdport, unsigned long Flags)
{
    ...
    switch (pme_opt[0])
    {
        ...
        case 'a': GMX_RELEASE_ASSERT(false, "Auto-choice not implemented"); break; // "auto" FIXME add heuristics below
	...
	// gmx_bcast_sim(sizeof(useGpuPME), &useGpuPME, cr);//FIXME this is overwritten
        ...
        if (cr->duty & DUTY_PME)
        {
            try
            {
                status = gmx_pme_init(pmedata, cr, npme_major, npme_minor, inputrec,
                                      ...
                                      nthreads_pme,
                                      pmeRunMode, nullptr, gpuTasks.gpuInfo(GpuTask::PME), mdlog, multipleContexts);
            }
            GMX_CATCH_ALL_AND_EXIT_WITH_FATAL_ERROR;
            if (status != 0)
            {
