...
\iffalse
Apart from the multiplication in the Fourier space, which is straightforward to vectorize, designing good algorithms for the other components is non-trivial.
On the CPU most simulation packages have converged to the same approach. But on GPU most components can not take advantage of SIMD instructions of width 8 (AVX) or more (AVX512). 
\fi
...
Almost like before, the asterisk in \eqref{eq:Edir} means that terms with $\bold{n} = \mathbb{0}$ and $i = j$, or $(i,j)\in M$, are skipped.
...
%It is shown in \draft{???} that the piece-wise interpolation of the exponentials is possible, using the Lagrange polynomials of order $p$:
...
\item Perturb the grid, calculate the reciprocal energy contribution (equation \eqref{Erecfull}).
...
\section{MD step}
\FloatBarrier
The core timeline of a GROMACS MD step is presented on a figure \ref{fig:step-orig}. In a typical use case, as the process has to calculate all the forces and energies, it first launches the more exhausting non-bonded work on a GPU, and only then performs the CPU calculations of the bonded and reciprocal forces. 
\begin{figure}
    \centering
...
\iffalse
The system is partitioned over the processes at the beginning of each MD step in which \draft{neighbour
...
Texture memory - a read only GPU memory bank, optimized for 2D access pattern. Uses smaller cache lines with comparison to the global memory.
...
\item Reads and writes have to be as coalesced as possible, obviously.
\item The possibility of the \draft{global load optimizations} is also available for a single-dimension data.
\end{enumerate}
...
%Funny, but now the wrapping kernel has excessive modulus operations because I am an idiot.\\
...
The algorithm needs to perturb each cell of the Fourier space complex grid, as well as compute the reciprocal energy and the virial contribution. This kernel only needs about \draft{1\%} of the total run-time, and on top of that is embarrassingly parallel. The only common data is the B-spline moduli for all 3 dimensions, stored in a texture memory, and the resulting global memory energy and $3 \times 3$ pressure virial (represented by 6 values because it is symmetric). The energy and the pressure virial are reduced block-wise in the shared memory, and then the result is atomically added to the global memory.
% \draft{This kernel is compute-bound or smem bpound?}

\section{Unwrapping the grid}
After the grid is perturbed, and complex-to-real FFT is performed, the long-range force contributions are calculated.

Just like the Cartesian space grid was being wrapped around after the spreading, it gets unwrapped before the force gathering. This does not require atomic operations, it is just a memory copying.

\section{Force calculation}
Force contribution gathering is a logical counterpart to the charge spreading. The values of the same grid cells that were affected by charge spreading are now being summed up to get the long-range force contribution for the corresponding particles.
...
The modified MD step in this implementation is launching the largest chunk of the PME work on a GPU before the NB work is launched (figure \ref{fig:step-gpu}). The PME stream is also set to high priority, using the CUDA runtime API, while the NB stream has lower priority. 
...
gromacs.org in bibliography\and my repository
perturbation vs convolution
\fi
