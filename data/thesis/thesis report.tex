...
%\draft{describe cuFFT vs FFTw in implem} 
...
The PME part is computed in one case on the same GPU, and in another case on the CPU (Intel Core i7-5960X). The number of the threads used by the process is changed from 1 to 8.
...
For the next comparison we take the same machine, but run a simulation consisting of 2 separate processes: 1 PP and 1 PME process. The PP process performs the short-range computations on the high-end NVIDIA GPU GTX Titan X, as before.    
The PME process is ran in one case on the CPU (Intel Core i7-5960X), and in another case on the second NVIDIA GPU Quadro M6000. The number of the threads used by the processes is changed from 1 to 8.
