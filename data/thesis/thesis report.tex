...
\end{minipage}
\caption{Example of varying convergence of the Ewald sum components using the scaling parameter; parameter values 0.7 (left) and 2 (right)}
\label{fig:split}
\end{figure}
%}
 
In many MD simulations certain \draft{non-}bonded interactions within same molecule are being omitted, being handled by different terms in the potential calculation. The corresponding particle pairs $(i, j)$ are stored in a masked list $M$. Therefore, the masked list correction should be subtracted from the direct and reciprocal energy sums. Usually the masked pairs are explicitly skipped over during the direct sum calculation.	 The other contribution to the correction term is the charges acting on self.
...
\end{equation}
are the discrete grid values. %\draft define m
